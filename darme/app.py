...
diseases_list = list(label_encoder.classes_)
...
def predict():
    try:
        ...
        desc = description_df[description_df['Disease'] == predicted_disease]
        if not desc.empty:
            desc = desc['Description'].values[0]
        else:
            desc = "Description not available."

        pre = precautions_df[precautions_df['Disease'] == predicted_disease]
        if not pre.empty:
            pre = pre.iloc[0].dropna().tolist()[1:]  # Remove NaN values and convert to list
        else:
            pre = ["Precautions not available."]

        med = medications_df[medications_df['Disease'] == predicted_disease]
        if not med.empty:
            med = med['Medication'].dropna().tolist()  # Ensure it's a list
        else:
